...
- **GraphQL executes in-process, not over HTTP.** `apollo-rsc.ts` links to `graphql-executor.ts` (shared PostGraphile preset + grafast); the server needs `POSTGRES_URL` (falls back to the local docker DB in dev), NOT a running api. Schema builds once per process, pinned to `globalThis`.
- **Prerender scope is capped to limit Neon egress.** `generateStaticParams` prerenders only the 2 most recent seasons (each season page fans out to ~14 GraphQL queries); older seasons render on-demand (`dynamicParams`). **Preview builds skip prerender entirely** — every `generateStaticParams` is wrapped in `previewSafeStaticParams()` (`lib/static-params.ts`), which makes the export `undefined` on preview (returning `[]` is a cacheComponents build error); keep the wrapper when adding new param'd routes. Don't widen the cap without weighing build-time Neon data transfer.
