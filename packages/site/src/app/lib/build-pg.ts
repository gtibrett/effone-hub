...
export async function buildSeasonYears(): Promise<string[]> {
	const rows = await queryColumn<number>(`SELECT year FROM f1db.season ORDER BY year DESC`, [
...
export async function buildDriverRowIds(): Promise<string[]> {
	return await queryColumn<string>(`SELECT id FROM f1db.driver ORDER BY id`, ['max-verstappen']);
}

export async function buildTeamRowIds(): Promise<string[]> {
	return await queryColumn<string>(`SELECT id FROM f1db.team ORDER BY id`, ['red-bull']);
}

export async function buildCircuitRowIds(): Promise<string[]> {
	return await queryColumn<string>(`SELECT id FROM f1db.circuit ORDER BY id`, ['monza']);
}
...
export async function buildDriverName(
	rowId: string
...
export async function buildTeamName(rowId: string): Promise<{ name: string } | null> {
	return await queryRow<{ name: string }>(`SELECT name FROM f1db.team WHERE id = $1`, [rowId]);
}

export async function buildRaceName(
	season: number,
	round: number
): Promise<{ officialName: string } | null> {
		...
		const buildTeamNode = (t: (typeof teamRows)[number]) => ({
			...
			team: t.team_id
				? {
						...
						rowId: t.team_id,
						colors: t.primary_hex
							? {
									__typename: 'AppTeamColor' as const,
									id: `pg:tc:${t.team_id}`,
									primaryHex: t.primary_hex
								}
							: null
					}
				: null
