...
const buildStrokeVars = (variant: CircuitMapProps['variant'], sector: string | undefined): CSSProperties => {
...
export default function CircuitMap({variant = 'interactive', circuitRef, height, width, ...svgProps}: CircuitMapProps) {
	...
	if (variant === 'simple') {
		return <Suspense><Box className={styles.map} style={strokeVars}>{mapSVG}</Box></Suspense>;
	}
            ...
            <Grid container spacing={2} className="justify-end">
				<Grid>
					<ToggleButtonGroup size="small" value={sector} onChange={handleChange} exclusive aria-hidden>
						<ToggleButton value="1"><FontAwesomeIcon icon={faSquareFull} style={{color: 'var(--map-sector-1)'}}/>&nbsp;Sector 1</ToggleButton>
						<ToggleButton value="2"><FontAwesomeIcon icon={faSquareFull} style={{color: 'var(--map-sector-2)'}}/>&nbsp;Sector 2</ToggleButton>
						<ToggleButton value="3"><FontAwesomeIcon icon={faSquareFull} style={{color: 'var(--map-sector-3)'}}/>&nbsp;Sector 3</ToggleButton>
					</ToggleButtonGroup>
				</Grid>
				<Grid className={styles.map} style={strokeVars} size={12}>{mapSVG}</Grid>
			</Grid>
        </Suspense>
    );
};
