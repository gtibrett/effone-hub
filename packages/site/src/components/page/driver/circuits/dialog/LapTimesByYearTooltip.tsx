...
type LapTimesByYearTooltipProps = Pick<
	ComputedBoxPlotSummary,
	'color' | 'data' | 'formatted' | 'label'
>;
		...
		<Card>
			...
			<PropertiesTable>
				<PropertiesTableRow header="Fastest">
					{getTimeStringFromDate(new Date(data.extrema[0]))}
				</PropertiesTableRow>
				<PropertiesTableRow header="10th %">
					{getTimeStringFromDate(new Date(data.values[0]))}
				</PropertiesTableRow>
				<PropertiesTableRow header="25th %">
					{getTimeStringFromDate(new Date(data.values[1]))}
				</PropertiesTableRow>
				<PropertiesTableRow header="50th %">
					{getTimeStringFromDate(new Date(data.values[2]))}
				</PropertiesTableRow>
				<PropertiesTableRow header="75th %">
					{getTimeStringFromDate(new Date(data.values[3]))}
				</PropertiesTableRow>
				<PropertiesTableRow header="90th %">
					{getTimeStringFromDate(new Date(data.values[4]))}
				</PropertiesTableRow>
				<PropertiesTableRow header="Slowest">
					{getTimeStringFromDate(new Date(data.extrema[1]))}
				</PropertiesTableRow>
			</PropertiesTable>
		</Card>
	);
