...
export default function useHistoryChartData(
	data: SimpleApolloResult<ConstructorPageData>['data']
...
export function useHistoryChartColors(chartData: HistoryChartData | undefined) {
	...
	return !chartData
		? []
		: Array.from(chartData.standingsByTeam.values()).map(t =>
				getTeamColor(
					t.colors ? { primaryHex: t.colors.primaryHex, secondaryHex: null } : null,
					'primaryHex'
				)
			);
}

export function getChartDataByAttribute(
	attribute: keyof TeamStandingData,
